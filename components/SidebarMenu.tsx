...
import { Menu, X, Trophy, ListChecks, Building2, GraduationCap, BarChart3, BedDouble, BriefcaseBusiness, Landmark } from "lucide-react";
...
export function SidebarMenu() {
  ...
  return (
    <>
      <button type="button" onClick={() => setOpen(true)} aria-label="Open menu" className="grid h-9 w-9 shrink-0 place-items-center rounded-full border border-white/10 bg-white/5 transition hover:scale-105 hover:bg-white/10">
        <Menu size={19} />
      </button>
        ...
        <div className="fixed inset-0 z-[100]">
          ...
          <aside className="absolute right-0 top-0 flex h-full w-[min(360px,88vw)] flex-col border-l border-white/10 bg-[#070707] p-6 shadow-2xl">
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={() => setOpen(false)} aria-label="Close menu" className="grid h-9 w-9 place-items-center rounded-full border border-white/10 bg-white/5 transition hover:bg-white/10">
                <X size={19} />
              </button>
            </div>
            ...
            <nav className="mt-8 flex flex-col gap-2">
              <Link href="/college-ranking" onClick={() => setOpen(false)} className="group rounded-2xl border border-[#f4c542]/20 bg-[#f4c542]/[.07] p-4 transition hover:bg-[#f4c542]/[.12]">
                <div className="flex items-center gap-3">
                  ...
                  <div><p className="font-black">College Crowd Ranking</p><p className="mt-0.5 text-xs text-zinc-500">CSE Phase 1 OC-Boys benchmark</p></div>
                </div>
              </Link>
